...
//
//*****************************************************************************
__weak void CAN_RxCallback(uint16_t ID, uint8_t* data)
{
...
void CAN0_SendData(uint16_t id, uint8_t data[4]){
// in this case there is just one type, but you could accept multiple ID types
  CAN0_Setup_Message_Object(id, NULL, 4, data, XMT_ID, MSG_OBJ_TYPE_TX);
}
