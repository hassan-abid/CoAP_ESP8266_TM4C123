...
// step 2: AT+CWMODE=3 set wifi mode to access point and client (not an access point)
...
// step 3: AT+CWJAP="ValvanoAP","12345678"  connect to access point                // rounter and password
...
// AT+CIPMUX==1 set mode to multiple socket 
  if(ESP8266_SetConnectionMux(1)==0){ // multiple socket
...
// Input: IP address or web page as a string
// output: 1 if success, 0 if fail 
int ESP8266_MakeUDPConnection(void){
  int try=MAXTRY;
	char ID[] = "4";
	char IPaddress[] = "";
	char pcPort[] = "";
  ...
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+CIPSTART=%s,\"UDP\",\"%s\",%s,%s,0\r\n", ID, IPaddress, pcPort, esp8266Port);
    ESP8266SendCommand(TXBuffer);   // open and connect to a socket
    DelayMsSearching(8000);
    if(SearchFound) return 1; // success
    try--;
...
//---------ESP8266_SendUDP----------                                   #########
// Send a UDP packet to server 
// Input: UDP payload to send
// output: 1 if success, 0 if fail 
int ESP8266_SendUDP(char* fetch){
  volatile uint32_t time,n;
	char ID[] = "4";
  sprintf((char*)TXBuffer, "AT+CIPSEND=%s,%d\r\n", ID, strlen(fetch));
  ESP8266SendCommand(TXBuffer);  
  DelayMs(50);
  ESP8266SendCommand(fetch);
  ServerResponseSearchStart();
...
//---------ESP8266_CloseUDPConnection----------                        ##############
// Close UDP connection 
// Input: none
// output: 1 if success, 0 if fail 
int ESP8266_CloseUDPConnection(void){
  int try=1;
	char ID[] = "4";
  SearchStart("ok");
  while(try){
		sprintf((char*)TXBuffer, "AT+CIPCLOSE=%s\r\n", ID);
    ESP8266SendCommand(TXBuffer);   
    DelayMsSearching(4000);
    if(SearchFound) return 1; // success
    try--;
